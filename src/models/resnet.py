...
class ResNet(nn.Module):
    ...
    def __init__(self,opt, block, layers, num_classes=1000):
        self.inplanes = 64
        ...
        self.fc = AttrLayer(self.opt, inplanes = 512 * block.expansion)
...
class ResNetTemplet(nn.Module):
    ...
    def __init__(self, opt, block, layers):
        self.inplanes = 64
...
def resnet18(opt):
    ...
    """
    model = ResNet(opt, BasicBlock, [2, 2, 2, 2])
    # if pretrained:
    #     model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model


def Resnet18Templet(opt):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNetTemplet(opt,BasicBlock, [2, 2, 2, 2])
    # if pretrained:
